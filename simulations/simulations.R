...
for (i in 1:nComm) {
  ...
  starting <- lapply(1:nCores, function(i) {
    list("phi" = alpha * param_ests["phi", i] + (1 - alpha) * mean(param_ests["phi", -i]), 
         "sigma.sq" = alpha * param_ests["sigma.sq", i] + (1 - alpha) * mean(param_ests["sigma.sq", -i]),  
         "tau.sq" = alpha * param_ests["tau.sq", i] + (1 - alpha) * mean(param_ests["tau.sq", -i]))
    })
}



sapply(1:nCores, \(i) apply(obj[[i]]$p.theta.samples, 2, mean))
